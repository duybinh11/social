...
const App: FC = (): ReactElement => {
    ...
    const [colorScheme, setColorScheme] = useState<ThemeOptions>(yellowColor as ThemeOptions);
    const [theme, setTheme] = useState<Theme>(lightsOutTheme);
    ...
    useEffect(() => {
        ...
        if (!localStorage.getItem("background")) {
            localStorage.setItem("background", BackgroundTheme.LIGHTS_OUT);
        }
        if (!localStorage.getItem("color")) {
            localStorage.setItem("color", ColorScheme.YELLOW);
        }
        processColorScheme(color as ColorScheme);
